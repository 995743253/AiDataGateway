...
public static class MetricCatalog
{
    ...
    [
        ...
        new("disk.volume_count", "本地磁盘数", "存储", "count", "当前已就绪且可采集的本地磁盘数量。"),
        ...
        new("network.interface_count", "网卡数量", "网络", "count", "系统中可见的网络接口数量。"),
        new("network.active_interface_count", "活动网卡数量", "网络", "count", "当前处于启用状态且非回环的网络接口数量。"),
        ...
        new("process.paged_memory_bytes", "Agent 分页内存", "采集进程", "bytes", "网关或远端 Agent 进程当前分页内存大小。"),
        new("process.virtual_memory_bytes", "Agent 虚拟内存", "采集进程", "bytes", "网关或远端 Agent 进程当前虚拟地址空间大小。"),
        ...
        new("system.thread_count", "系统线程数", "系统", "count", "当前服务器所有可访问进程的线程总数；仅在启用后采集，个别受保护进程可能不计入。"),
        new("system.handle_count", "系统句柄数", "系统", "count", "当前服务器所有可访问进程的句柄总数（Windows 最完整）。"),
        ...
        new("system.tcp_established_count", "已建立 TCP 连接数", "系统", "count", "当前处于 Established 状态的 TCP 连接数量。"),
        new("system.udp_listener_count", "UDP 监听数", "系统", "count", "当前活动 UDP 监听端点数量。"),
        ...
        new("gc.fragmented_bytes", ".NET GC 碎片", "运行时", "bytes", "最近一次 GC 后托管堆中的碎片大小。"),
        new("gc.committed_bytes", ".NET GC 已提交内存", "运行时", "bytes", "运行时已为 GC 堆提交的内存。"),
        new("gc.gen0_collection_count", "Gen 0 回收次数", "运行时", "count", "本进程启动后发生的第 0 代 GC 次数。"),
        new("gc.gen1_collection_count", "Gen 1 回收次数", "运行时", "count", "本进程启动后发生的第 1 代 GC 次数。"),
        new("gc.gen2_collection_count", "Gen 2 回收次数", "运行时", "count", "本进程启动后发生的第 2 代 GC 次数。")
    ];
