...
密码至少 6 位，可以使用中文、英文、数字或符号，不再强制大小写与数字组合。
...
| 表黑名单 | 可选；每行填写一个禁止 AI 查询的表名，例如 `AspNetUsers` 或 `dbo.Secrets` |
| 最大返回行 | 建议先设置为 500 或 1000 |
| 超时秒数 | 一般使用 30 秒 |

保存后点击“测试”。看到连接成功，才表示网关能够访问该数据库。

黑名单按数据源分别生效。SQL 的 `FROM`、`JOIN` 或子查询只要引用黑名单表，网关就会在连接目标数据库前拒绝请求，并在运行日志中记录“黑名单拦截查询”。
...
用户可编辑角色和启用状态。已有审批或审计历史的账号不能永久删除，应改为禁用；当前登录账号和最后一个管理员也不能删除。
...
不再使用的客户端应点击“吊销删除”。删除后不能再申请 Token，已经签发的 Token 也会立即失效。
...
清理不会删除用户、数据源、OAuth2 客户端，也不会操作任何业务数据库。

如果每日计划时间软件没有运行，不需要补开等待；下次打开软件时会立即完成一次清理。
