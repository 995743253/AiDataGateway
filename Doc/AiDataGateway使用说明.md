...
“应用日志”页面按日期统一显示结构化事件，包括多行消息、异常、空字段、原始记录和不完整记录提示；选择日志源后可直接点击“查询”，不需要先执行连接测试。Seq 默认提供关键词/属性简易查询，也保留高级表达式模式。新增独立“实时日志”页面，通过 SSE 接收新事件并最多保留 500 条。

本地文本日志还会把消息前缀、`key=value` 和消息内 JSON 顶层字段提取成键值对，双击日志可在“结构化属性”中查看。
