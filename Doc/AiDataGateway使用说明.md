...
勾选“记住密码”后，程序使用受保护的登录 Cookie 在本机保持登录 30 天，不会把明文密码写入网页存储；未勾选时只保持当前 WebView/浏览器会话。
...
如果忘记 Administrator 账号的登录密码，可在登录页点击“忘记管理员密码？”：
...
首次安装的默认重置口令是 `admin`。管理员第一次登录后应进入“系统设置 → 管理员密码恢复”立即修改。自定义重置口令经本机 Data Protection 加密后保存在内置数据库中，页面和 API 均不会返回原值。

为避免重复尝试，重置接口每分钟最多接受 5 次请求。错误的重置口令、成功重置和重置口令变更都会进入网关审计日志。该功能只能重置已启用且拥有 `Administrator` 角色的账号，不能用于普通用户。

## 6. 数据源管理

在“数据源”页面点击“新增数据源”。每行提供“审批记录”按钮，可直接跳转到审批记录页并自动只显示该数据源的工单；页面顶部会显示“数据源：名称”标签，点 × 或“重置”即可取消过滤（需要 Administrator/Approver 权限）。
...
“项目”页面用唯一项目编号聚合多个数据源、日志源和监控节点；项目与三类资源均为多对多关系。“日志源”支持本机 NLog、Seq HTTP API 和远程采集 Agent；每行提供“应用日志”按钮，点击后直接跳转到应用日志页、选中该日志源并自动执行查询（已禁用的日志源会提示先启用）。配置与查询方法见 [项目与日志接入说明](./项目与日志接入说明.md)。
...
“应用日志”页面按日期统一显示结构化事件，包括多行消息、异常、空字段、原始记录和不完整记录提示；选择日志源后可直接点击“查询”，不需要先执行连接测试。工具栏的“实时日志 →”按钮可携带当前日志源跳转到实时日志页。本地文本日志只按日志源配置的 Layout 字段解析，消息原文（含 SQL）不会被拆解成属性。Seq 默认提供关键词/属性简易查询，也保留高级表达式模式。新增独立“实时日志”页面，通过 SSE 接收新事件并最多保留 500 条。
